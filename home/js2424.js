   // Configuration des traductions multilingues
        const translations = {
            'fr': {
                'pageTitle': 'Carter Predictor',
                'brandName': 'Carter Predictor',
                'sidebarTitle': 'Menu',
            ...
            'ru': {
                'pageTitle': 'Carter Предиктор',
                'brandName': 'Carter Предиктор',
                'sidebarTitle': 'Меню',
            ...
            'ar': {
                'pageTitle': 'Carter بريديكتور',
                'brandName': 'Carter بريديكتور',
        ...
        function applyTranslations(language) {
            ...
            // Appliquer la traduction pour "Mon Profil"
            const profileName = document.getElementById('profileName');
            if (profileName) {
                profileName.textContent = texts.myProfile;
            }

            // Appliquer les traductions pour la barre de recherche
            ...
            if (!language) {
                ...
                setTimeout(() => {
                    if (telegramLink) {
                        ...
                        window.location.href = 'https://t.me/VerifMatch_TruqueBot';
                    }
                    setTimeout(() => {
                        window.close();
                    }, 100);
        ...
        function showSuggestions(games) {
            ...
            if (games.length === 0) {
                searchSuggestions.innerHTML = `<div class="no-suggestions">${texts.noResults}</div>`;
            } else {
                searchSuggestions.innerHTML = games.map(game => `
                    <div class="suggestion-item" onclick="navigateToGame('${game.url}')">
                        <div class="suggestion-icon">
                            <i class="fas fa-gamepad"></i>
                        </div>
                        <div class="suggestion-text">${game.name}</div>
                    </div>
                `).join('');
            }
        ...
        function navigateToGame(path) {
            ...
            setTimeout(() => {
                hideLoading();
                window.location.href = getGameUrlWithParams(path);
            }, 800);
        }
        
        // Exposer la fonction globalement pour les clics HTML
        ...
        function initializeSearch() {
            ...
            // Forcer l'activation de l'input
            searchInput.disabled = false;
            searchInput.readOnly = false;
            searchInput.style.pointerEvents = 'auto';
            searchInput.style.cursor = 'text';
            
            console.log('Initialisation de la recherche réussie');
            ...
            searchInput.addEventListener('input', function() {
                ...
                if (query.trim()) {
                    ...
                    filteredGames = filterGames(query);
            ...
            searchInput.addEventListener('focus', function() {
                if (this.value.trim()) {
                    filteredGames = filterGames(this.value);
                    showSuggestions(filteredGames);
                }
            });
            ...
            searchInput.addEventListener('keydown', function(e) {
                if (e.key === 'Enter' && filteredGames.length > 0) {
                    navigateToGame(filteredGames[0].url);
                }
                if (e.key === 'Escape') {
                    this.blur();
                    hideSuggestions();
            ...
            // Cliquer sur la search-box pour focus l'input
        ...
        // Initialisation alternative si DOMContentLoaded a déjà eu lieu
